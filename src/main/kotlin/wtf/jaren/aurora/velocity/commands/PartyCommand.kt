...
import wtf.jaren.aurora.velocity.enums.PartyAddReason
import wtf.jaren.aurora.velocity.enums.PartyRemoveReason
import wtf.jaren.aurora.velocity.enums.PunishmentType
import wtf.jaren.aurora.velocity.managers.PartyManager
import wtf.jaren.aurora.velocity.objects.Party
import wtf.jaren.aurora.velocity.utils.BrigadierUtils
import wtf.jaren.aurora.velocity.utils.canSee
import wtf.jaren.aurora.velocity.utils.displayNameFor
import java.util.concurrent.CompletableFuture

class PartyCommand(private val plugin: Aurora) {
    ...
    fun register() {
        ...
        val totalNode = LiteralArgumentBuilder
            ...
            .then(LiteralArgumentBuilder
                ...
                .requires { source: CommandSource -> source.hasPermission("aurora.party.add") }
                .then(RequiredArgumentBuilder.argument<CommandSource, String>("player", StringArgumentType.string())
                    .suggests { obj: CommandContext<CommandSource>, context: SuggestionsBuilder ->
                        BrigadierUtils.getSuggestionsWithoutSender(obj, context)
                    }
    ...
    private fun join(context: CommandContext<CommandSource>): Int {
        ...
        if (party == null || !(party.isInvited(player) || party.public || player.hasPermission("aurora.party.override"))) {
    ...
    private fun promote(context: CommandContext<CommandSource>): Int {
        ...
        if (!(party.leader === player || player.hasPermission("aurora.party.override"))) {
